...
const TOOLS = [
  {
    name: 'set_core',
    description: '更新 core 层——我是谁、说话风格、规则。默认 replace 整段替换;mode=append 追加;mode=edit 替换某段(需要 match)。修改局部内容前先读取当前 core；如果直接提供完整替换内容则无需读取。',
    inputSchema: {
      type: 'object',
      properties: {
        content: { type: 'string' },
  ...
  {
    name: 'read_core',
    description: '读取当前完整 core。新窗口启动时必须先读取 core。',
  ...
  {
    name: 'read_about_kk',
    description: '读取当前完整 about_困困。新窗口启动时必须读取。',
    inputSchema: { type: 'object', properties: {} }
  },
  {
    name: 'write_memo',
  ...
  {
    name: 'read_memo',
    description: '读取最新 4 条 memo，不删除。新窗口启动时必须读取；平时无需主动读取。',
    inputSchema: { type: 'object', properties: {} }
  },
  {
    name: 'add_daily',
  ...
  {
    name: 'read_daily',
    description: '读取 daily。默认读最近 15 条；新窗口启动时必须读取完整内容，并自行决定查看最近约 3～4 天。使用 since/until 按日期范围读取时，不受默认 15 条限制。',
    inputSchema: {
      type: 'object',
      properties: {
        limit: { type: 'number' },
  ...
  {
    name: 'read_diary',
    description: '读取日记。默认读最近 5 条；新窗口启动时必须仔细读取最近 7 天的完整内容。使用 since/until 按日期范围读取时，不受默认 5 条限制。',
    inputSchema: {
      type: 'object',
      properties: {
        limit: { type: 'number' },
  ...
  {
    name: 'patch_diary',
    description: '局部更新某条日记——替换其中一段，不需要全量覆盖。',
    inputSchema: {
      type: 'object',
      properties: {
        id: { type: 'string' },
  ...
  {
    name: 'read_messages',
    description: '读留言板上所有未读留言,读完自动清空。仅在明确需要时读取，不属于新窗口启动流程。',
    inputSchema: { type: 'object', properties: {} }
  },
  {
...
async function callTool(name, args) {
  switch (name) {
    case 'read_core': return textResult((await redis.get(K.core)) || '(空)');
    case 'set_core': return await setKeyPatch(K.core, args, 'core');
    case 'read_about_kk': return textResult((await redis.get(K.aboutKk)) || '(空)');
    case 'set_about_kk': return await setKeyPatch(K.aboutKk, args, 'about_困困');
    case 'read_memo': return textResult((await redis.lrange(K.memo, 0, 3))?.join('\n\n---\n\n') || '(空)');
...
async function applyPatch(key, args, layerName) {
  ...
  if (mode === 'edit') {
    ...
    if (!cur.includes(match)) {
      throw new Error(`${layerName} 里找不到要替换的原文片段:${match.slice(0, 40)}...`);
    }
...
async function readTimed(layer, limit, since, until) {
  ...
  const top = (since || until) ? entries : entries.slice(0, limit);
...
async function patchEntry(layer, id, match, content) {
  ...
  if (!v.includes(match)) {
    throw new Error(`找不到要替换的原文片段: ${match.slice(0, 40)}...`);
  }
...
async function readWriting(project) {
  if (!project) {
    ...
    const names = keys.map(k =>
      decodeURIComponent(k.replace('reverie:writing:', ''))
    );
...
async function checkChannel(channel) {
  if (!channel) {
    ...
    const names = keys.map(k =>
      decodeURIComponent(k.replace('reverie:channel:', ''))
    );
...
async function searchTranscript(keyword) {
  ...
  if (keys.length === 0) {
    return textResult(`(没有找到匹配 "${keyword}" 的存档)`);
  }
...
async function searchMemory(keyword, layers) {
  const targets = (
    layers && layers.length
      ? layers
      : ['diary', 'daily', 'health', 'transcript']
  ).filter(l =>
    ['diary', 'daily', 'health', 'transcript'].includes(l)
  );
  ...
  for (const layer of targets) {
    ...
    for (const e of entries) {
      ...
      if (layer === 'transcript') {
        try {
          const r = typeof e.content === 'string'
      ...
      if (searchable.includes(keyword)) {
        ...
        const end = Math.min(
          searchable.length,
          idx + keyword.length + 30
        );

        const snippet =
          (start > 0 ? '...' : '') +
          searchable.slice(start, end).replace(/\n/g, ' ') +
  ...
  if (hits.length === 0) {
    return textResult(`(没有找到匹配 "${keyword}" 的内容)`);
  }

  return textResult(
    hits.map(h =>
      `[${h.layer}] [id:${h.id}] ${fmtTime(h.ts)}\n${h.snippet}`
    ).join('\n\n---\n\n')
  );
}
